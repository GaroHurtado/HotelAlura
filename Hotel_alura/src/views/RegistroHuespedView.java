...
import Controllers.RegistroHuespedController;
import Controllers.ReservasController;
import Dao.RegistroHuespedDao;
...
public class RegistroHuespedView extends JFrame {
	...
	private RegistroHuespedController controller;
	private RegistroHuespedDao registroDao;
	private ReservasController numReserva;
	int xMouse, yMouse;
	...
	public RegistroHuespedView() {
		...
		btnAtras.setBackground(new Color(18, 59, 98));
		...
		txtFechaN.getCalendarButton().addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if(txtNombre!=null && txtApellido!=null) {
					numReserva = new ReservasController();
					txtNreserva.setText("Numero de reservacion: " + String.valueOf(numReserva.numeroReserva()));
		...
		lblTitulo.setForeground(new Color(18, 59, 98));
		...
		btnguardar.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				controller = new RegistroHuespedController(String.valueOf(txtFechaN.getDate()));
				registroHuesped = new RegistroHuesped(txtNombre.getText(), 
										txtApellido.getText(), 
										controller.modificarFecha(), 
										String.valueOf(txtNacionalidad.getSelectedItem()), 
										txtTelefono.getText());
				
				registroDao = new RegistroHuespedDao();
				
				if(txtNombre != null && txtApellido != null && txtFechaN!=null && txtTelefono != null) {
					registroDao.guardarBase(registroHuesped);
					JOptionPane.showMessageDialog(null,  "Registro Exitoso");
					JOptionPane.showMessageDialog(null,  "Muchas Gracias por Hospedarse con nosotros");
					EventQueue.invokeLater(new Runnable() {
						public void run() {
							try {
					...
					JOptionPane.showMessageDialog(null, "ATENCION", "FALTA INFORMACION", JOptionPane.INFORMATION_MESSAGE);
				}
				
				System.out.println(txtFechaN.getDate());
				System.out.println(controller.modificarFecha());
			}
		});
		btnguardar.setLayout(null);
		btnguardar.setBackground(new Color(18, 59, 98));
		...
		imagenFondo.setBounds(0, 0, 489, 634);
		panel.add(imagenFondo);
		imagenFondo.setIcon(new ImageIcon(RegistroHuespedView.class.getResource("/ImagenesIA/image (10).png")));
